...
-- Approach 6: Delta Live Tables (DLT) with APPLY CHANGES
-- Full medallion pipeline using the CLASSIC DLT SQL syntax (pre-rebrand).
-- Contrast with Approach 4 which uses the modern Declarative Pipelines syntax.
--
-- Classic DLT keywords:
--   CREATE STREAMING LIVE TABLE  (modern: CREATE OR REFRESH STREAMING TABLE)
--   CREATE LIVE TABLE            (modern: CREATE OR REFRESH MATERIALIZED VIEW)
--
-- SCD2 source reads from STREAM(LIVE.bronze_customers) rather than a separate
-- STREAM read_files() call (contrast with Approach 4).
--
-- APPLY CHANGES produces __START_AT/__END_AT columns instead of
...
-- =============================================
-- Bronze Layer: Streaming Live Tables from landing volume
-- =============================================
...
CREATE STREAMING LIVE TABLE bronze_order_lines
AS SELECT
  order_id,
  line_id,
  product_id,
...
-- =============================================
-- Silver Layer: Live Tables with expectations
-- =============================================
...
CREATE LIVE TABLE silver_products (
...
CREATE LIVE TABLE silver_order_lines (
...
-- Sources from STREAM(LIVE.bronze_customers) with inline transforms + _batch_date.
-- Contrast with Approach 4 which reads directly from STREAM read_files().
-- TRACK HISTORY ON limits SCD2 triggers to the same columns as other approaches.

CREATE STREAMING LIVE TABLE _scd2_dim_customer;
...
FROM (
  SELECT
    ...
    INITCAP(TRIM(segment)) AS segment,
    CASE
      WHEN _batch_id = 'batch_1' THEN DATE '2024-01-01'
...
CREATE LIVE TABLE gold_dim_product
AS
SELECT
...
CREATE LIVE TABLE gold_fact_order_line
AS
SELECT
